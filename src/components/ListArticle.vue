                  ...
                  <table class="table table-striped">
                      ...
                      <tbody> 
                          <tr v-for="tableData in tableData" :key="tableData.createByBlogUserId">
                            ...
                            <td>{{ tableData.shortDescription }}</td>
                            <td>
...
export default {
  ...
  methods: {
    testApi() {
      ...
      axios.get(`https://localhost:7185/api/Admin/User/${this.Id}/Article?PageNumber=${this.currentPage}`, { headers: headers })
        .then((res) => {
    ...
    Reload(){
     ...
     DeleteArticle(idArtilce){
      var token = sessionStorage.getItem("JWT");
      const headers = {
        'Authorization': "Bearer " + token,
      };
       if (window.confirm("Do you really want to delete?")) {
        axios.delete(`https://localhost:7185/api/Admin/User/${this.Id}/Article/${idArtilce}`, { headers: headers })
        .then((res) => {
          this.Reload();
          console(res);
        })
